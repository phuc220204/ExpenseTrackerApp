...
export const useAIChat = () => {
  ...
  const handleSendMessage = async (userMessage) => {
    ...
    try {
      ...
      const MAX_HISTORY = 8;
      const limitedHistory = messages.length > MAX_HISTORY 
      ...
      if (aiResponse.functionCalls && aiResponse.functionCalls.length > 0) {
        ...
        if (addTransactionCalls.length > 0) {
          ...
          if (transactions.length === 1) {
            ...
            const previewMessage = aiResponse.text || 
              `Đã chuẩn bị giao dịch ${transactions[0].type === "income" ? "thu nhập" : "chi tiêu"} ${transactions[0].amount?.toLocaleString("vi-VN")} VND.\n\nVui lòng xác nhận bên dưới để lưu vào hệ thống.`;
      ...
      let errorMessage = "Xin lỗi, có lỗi xảy ra khi xử lý yêu cầu của bạn. Vui lòng thử lại.";
      
      if (error.message?.includes("429") || error.message?.includes("quota") || error.message?.includes("rate-limit")) {
        errorMessage = "⚠️ Bạn đã vượt quá hạn mức sử dụng API (quota) của Gemini.\n\n" +
          "Vui lòng:\n" +
        ...
        errorMessage = "❌ Model AI không tìm thấy.\n\n" +
  ...
  const handleConfirmAdd = async () => {
    ...
    try {
      ...
      addMessage(
        "assistant",
        "✅ Đã lưu giao dịch thành công vào hệ thống!"
      );
    } catch (error) {
      console.error("Lỗi khi lưu giao dịch:", error);
      addMessage(
